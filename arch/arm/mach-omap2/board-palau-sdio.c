/* arch/arm/mach-omap2/board-palau-sdio.c
 *
...
static struct omap2_hsmmc_info palau_mmc_info[] = {
	{
		.mmc		= 2,
		.nonremovable	= true,
		.external_ldo	= true,
...
static int palau_hsmmc_late_init(struct device *dev)
{
	int ret = 0;
...
static void __init palau_hsmmc_set_late_init(struct device *dev)
{
	...
	pdata->init = palau_hsmmc_late_init;
}

static int __init palau_hsmmc_init(struct omap2_hsmmc_info *controllers)
{
	...
	for (c = controllers; c->mmc; c++)
		palau_hsmmc_set_late_init(c->dev);

	return 0;
}

void __init omap4_palau_sdio_init(void)
{
	palau_hsmmc_init(palau_mmc_info);
}
